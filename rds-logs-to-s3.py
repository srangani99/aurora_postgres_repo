#This script will push RDS logs to S3 based on arguments. Initially it will push past 24 hours of logs and then everytime with incremental RDS logs.
...
def copy_logs_from_RDS_to_S3():
	...
	while moreLogsRemaining:
		...
		for dbLog in dbLogs['DescribeDBLogFiles']:
			...
			if partialData == True:
				# Edge case where DBlogFile rotates while being downloaded
				objectName = RDSInstanceName + "/" + "rdslogbackup" + "/" + dbLog['LogFileName'] + "error"
			else:
				objectName = RDSInstanceName + "/" + "rdslogbackup" + "/" + dbLog['LogFileName']
			try:
	...
	objectName = RDSInstanceName + "/" + "rdslogbackup"  + "/" + activityLogFileName
